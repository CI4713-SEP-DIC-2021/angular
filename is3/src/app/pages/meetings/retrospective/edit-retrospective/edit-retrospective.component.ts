...
@Component({
  selector: 'app-edit-retrospective',
  templateUrl: './edit-retrospective.component.html',
  styleUrls: ['./edit-retrospective.component.scss']
})
export class EditRetrospectiveComponent implements OnInit {
  ...
  constructor(
    ...
    private dialogRef: MatDialogRef<EditRetrospectiveComponent>,
  ...
  planningForm = this.formBuilder.group({
    ...
    positive: [this.data?.positive, Validators.required],
    negative: [this.data?.negative, Validators.required],
    decision: [this.data?.decision, Validators.required],
  ...
  addRetrospective() {
    ...
    this.meetingService.updateRetrospective(this.planningForm.value, this.data?.id);
